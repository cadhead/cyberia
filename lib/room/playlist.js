...
import { currentVideoTimeLeft } from './video';
...
export const playListItemSetCurrent = ({ room }, item) => {
  ...
  currentVideoTimeLeft[uniqName] = 0;

  if (!previousItem.isPinned && previousItem !== currentItem) {
    // eslint-disable-next-line no-use-before-define
    playlistItemRemove({ room }, previousItem);
  }
...
export const playlistItemAdd = async ({ socket, room }, item) => {
  ...
  if (isItemAlreadyInPool(room, item)) return null;
...
export const playlistItemRemove = async ({ room }, item) => {
  const { url } = item;
  const { uniqName } = room;
  const candidateItem = playlistItems[uniqName].find((it) => it.url === url);
  ...
  if (candidateItem) {
    ...
    if (!currentItem) {
      if (!playlistItems[uniqName][index]) return;
