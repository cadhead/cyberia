...
import {
  playlistItemAdd, playlistItemRemove, playlistItems, playListItemSetCurrent, playlistItemSetNext, playListItemTogglePin
} from './playlist';
...
export default {
  'user:join_room': ({ socket, room }) => {
    ...
    socket.emit('user:join_room', {
      ...
      lastPlaylistItems: playlistItems[room.uniqName] || []
    });
  },
  ...
  'playlist:play': async ({ socket, room }, item) => {
    playListItemSetCurrent({ room }, item);

    socket.broadcast.emit('playlist', playlistItems[room.uniqName]);
    socket.emit('playlist', playlistItems[room.uniqName]);
  },
  'playlist:pin': async ({ socket, room }, item) => {
    playListItemTogglePin({ room }, item);

    socket.broadcast.emit('playlist', playlistItems[room.uniqName]);
    socket.emit('playlist', playlistItems[room.uniqName]);
  },
  'playlist:remove': async ({ socket, room }, item) => {
    playlistItemRemove({ room }, item);

    socket.broadcast.emit('playlist', playlistItems[room.uniqName]);
    socket.emit('playlist', playlistItems[room.uniqName]);
  },
  'playlist:setNext': async ({ socket, room }, item) => {
    playlistItemSetNext({ room }, item);
